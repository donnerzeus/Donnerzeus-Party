...
const Steering = ({ players, roomCode, onGameOver }) => {
    ...
    const [pagePhase, setPagePhase] = useState('countdown'); // countdown, playing
    ...
    useEffect(() => {
        ...
        const unsubscribe = onValue(roomRef, (snapshot) => {
            ...
            if (data && !finished && pagePhase === 'playing') {
        ...
        return () => unsubscribe();
    }, [roomCode, finished, onGameOver, pagePhase]);
            ...
            <AnimatePresence mode="wait">
                {pagePhase === 'countdown' && (
                    <motion.div key="cd" className="center-all" initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ opacity: 0, scale: 2 }}>
                        <Compass size={120} color="#00f2ff" className="glow-icon" />
                        <h1 className="neon-text title">CO-OP STEERING</h1>
                        <h1 className="big-cd">{countdown}</h1>
                        <div className="instruction-box glass-panel">
                            <p>TILT YOUR PHONES TOGETHER TO STEER THE ORB TO THE PORTAL!</p>
                        </div>
                    </motion.div>
                )}
