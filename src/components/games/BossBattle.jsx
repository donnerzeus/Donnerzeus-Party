...
const BossBattle = ({ players, roomCode, onGameOver }) => {
    ...
    const handlePlayerAction = (playerId, type) => {
        if (type === 'attack') {
            setBossHp(prev => Math.max(0, prev - 12));
            setBossState('hurt');
            addVfx(50, 40, 'hit');
            setTimeout(() => setBossState('idle'), 200);
        } else if (type === 'heal') {
            setTeamEnergy(prev => Math.min(100, prev + 8));
            addVfx(50, 80, 'heal');
        }
    };
    ...
    return (
        <div className={`boss-battle center-all ${shake ? 'screanshake' : ''} ${isEnraged ? 'enraged' : ''}`}>
            ...
            <AnimatePresence>
                {gameState === 'intro' && (
                    <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 2, opacity: 0 }} className="boss-intro center-all">
                        ...
                        <h1 className="neon-text giant">FINAL DEFIANCE</h1>
            ...
            <style>{`
                ...
                .bar-bg.giant { width: 600px; height: 30px; border: 2px solid rgba(255,255,255,0.1); box-shadow: inset 0 0 20px rgba(0,0,0,0.5); }
                .bar-fill.hp { background: linear-gradient(90deg, #ff0044, #880022); filter: drop-shadow(0 0 10px #ff0044); }
                ...
                .enraged-mode { filter: drop-shadow(0 0 80px #ff0000) brightness(1.5); }
                .boss-eye-glow { position: absolute; top: 40%; width: 200px; height: 60px; background: #ff0044; filter: blur(40px); opacity: 0.6; }
                .dark-fire { position: absolute; inset: -40px; background: radial-gradient(circle, rgba(255,255,255,0.1) 0%, transparent 70%); animation: flicker 0.05s infinite; }
                ...
                .screanshake { animation: shake 0.4s cubic-bezier(.36,.07,.19,.97) both; }
                ...
                .boss-result { position: absolute; inset: 0; background: rgba(0,0,0,0.9); z-index: 1000; }
